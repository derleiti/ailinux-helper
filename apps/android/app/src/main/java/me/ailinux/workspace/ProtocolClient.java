...
final class ProtocolClient extends WebSocketListener {
    ...
    static final String VERSION="2.90.10-android";
    ...
    private final TermuxShell shell;
    ...
    ProtocolClient(Context context, Listener listener){this.context=context.getApplicationContext();this.state=new StateStore(context);this.listener=listener;this.shell=new TermuxShell(this.context,this.state);}
    ...
    private JSONArray capabilities(){JSONArray out=new JSONArray();for(int i=0;i<readCaps.length();i++)out.put(readCaps.optString(i));if("write".equals(state.mode())){out.put("file_edit").put("directory_create").put("workspace_clear").put("code_edit");if(shell.released())out.put("shell");}return out;}
    ...
    private JSONObject execute(String tool,JSONObject args)throws Exception{switch(tool){case"workspace_info":return workspace.info(capabilities());case"file_read":case"code_read":return workspace.read(args);case"file_tree":case"code_tree":return workspace.tree(args);case"code_search":return workspace.search(args,false);case"code_grep":return workspace.search(args,true);case"file_edit":return workspace.edit(args);case"directory_create":return workspace.createDir(args);case"workspace_clear":return workspace.clear(args);case"file_ops":return workspace.fileOps(args);case"code_edit":return workspace.codeEdit(args);case"shell":return shell.run(args.optString("command",""),args.optString("cwd","."),args.optInt("timeout",120),state.tree());default:throw new IllegalArgumentException("unsupported Android workspace tool: "+tool);}}
