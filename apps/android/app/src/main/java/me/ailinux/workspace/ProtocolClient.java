...
final class ProtocolClient extends WebSocketListener {
    ...
    private JSONArray capabilities(){JSONArray out=new JSONArray();if(state.tree()!=null){for(int i=0;i<readCaps.length();i++)out.put(readCaps.optString(i));if("write".equals(state.mode()))out.put("file_edit").put("directory_create").put("workspace_clear").put("code_edit");}if(state.screenObserve()&&screenCapture!=null&&screenCapture.isReady())out.put("computer_observe").put("computer_screenshot");if(state.clipboardRead())out.put("clipboard_read");if(state.clipboardWrite())out.put("clipboard_write");if(state.remoteCompute())out.put("compute_execute");return out;}
    ...
    private JSONObject nativeShareProfile()throws Exception{JSONObject resources=new JSONObject().put("advertise",state.resourceAdvertise());if(state.resourceAdvertise())resources.put("inventory",deviceResources());boolean display=state.screenObserve()&&screenCapture!=null&&screenCapture.isReady();return new JSONObject().put("visibility",state.visibility()).put("clipboard",new JSONObject().put("read",state.clipboardRead()).put("write",state.clipboardWrite())).put("display",new JSONObject().put("observe",display).put("control",false)).put("resources",resources).put("compute",new JSONObject().put("remote_requested",state.remoteCompute()).put("runtime","triforce_docker").put("available",state.remoteCompute()).put("ephemeral",true).put("internet","public_only").put("workspace_path","~/workspace"));}
    private void connect(){
        if(stopped.get()||!connecting.compareAndSet(false,true))return;
        boolean nativeCapability=(state.screenObserve()&&screenCapture!=null&&screenCapture.isReady())||state.clipboardRead()||state.clipboardWrite();
