...
final class ProtocolClient extends WebSocketListener {
    ...
    static final String VERSION="2.90.19-android";
    ...
    private JSONArray capabilities(){JSONArray out=new JSONArray();if(state.tree()!=null){for(int i=0;i<readCaps.length();i++)out.put(readCaps.optString(i));if("write".equals(state.mode()))out.put("file_edit").put("directory_create").put("workspace_clear").put("code_edit");}if(state.screenObserve()&&screenCapture!=null&&screenCapture.isReady())out.put("computer_observe").put("computer_screenshot").put("vision_start").put("vision_status").put("vision_observe").put("vision_stop");if(state.clipboardRead())out.put("clipboard_read");if(state.clipboardWrite())out.put("clipboard_write");if(state.computerControl()&&DeviceControlService.isReady())out.put("computer_input").put("app_ops");if(state.remoteCompute())out.put("compute_execute");return out;}
